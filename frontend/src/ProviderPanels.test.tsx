...
import { GenerationDialog, GenerationDetails, ProvidersPanel, type ProviderInfo } from './ProviderPanels';
afterEach(()=>{cleanup();vi.restoreAllMocks();});
const provider:ProviderInfo={id:'openai',name:'OpenAI',enabled:true,health:'HEALTHY',defaultModel:'gpt-image-2',models:['gpt-image-2'],default:true,environment:'development',capabilities:{supportedFormats:['PNG','JPEG'],supportedQualities:['AUTO','LOW'],supportedSizes:['1024x1024'],arbitraryDimensions:false,supportsTransparentBackground:true},requestsPerMinute:20,maxConcurrent:2,stats:{requests_today:3,success_rate:66.67,average_latency_ms:1200,estimated_cost_today:null,unknown_cost_attempts:1}};
...
describe('Provider operations UI',()=>{
 ...
 it('shows fallback attempt history and requires acknowledgement of duplicate billing',async()=>{
  ...
  wrap(<GenerationDetails id="gen-1" close={()=>{}}/>);expect(await screen.findByText('TIMED_OUT')).toBeInTheDocument();expect(screen.getByRole('button',{name:'Retry generation'})).toBeDisabled();
  await userEvent.click(screen.getByRole('checkbox'));expect(screen.getByRole('button',{name:'Retry generation'})).toBeEnabled();expect(screen.getByText(/Cost unknown/)).toBeInTheDocument();
 });
});
