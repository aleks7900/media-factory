...
TASK-05 adds the [Similarity Engine](similarity-engine.md): indexed SHA/pHash retrieval, a long-lived local CLIP service behind a Java provider interface, immutable model-versioned pgvector embeddings, contextual classification, human duplicate review, collection clustering, and staged generation with diversity guards. TASK-04 [Advanced Visual QA](quality-assurance.md) consumes structured similarity findings; Java/database publication guards require the effective QA and similarity policies to pass. Local model inference happens outside transactions. The original sections below remain historical where superseded. See the [TASK-05 report](task-05-report.md) for exact model identity and verified runtime limits.
