...
public class AmoledAnalyzer {
  ...
  public Result analyze(byte[] bytes, Policy policy) {
    ...
    try (var input = ImageIO.createImageInputStream(new ByteArrayInputStream(bytes))) {
      ...
      if (!readers.hasNext()) throw new IllegalArgumentException("Unsupported AMOLED image");
  ...
  public Result analyze(BufferedImage image, Policy policy) {
    ...
    if (count > 64_000_000) throw new IllegalArgumentException("AMOLED image exceeds pixel limit");
    double sum = 0;
      ...
      for (int x = 0; x < image.getWidth(); x++) {
        ...
        double luminance =
            .2126 * linear[(rgb >> 16) & 255]
                ...
                + .0722 * linear[rgb & 255];
        if (luminance <= policy.blackMaximum()) black++;
        if (luminance <= policy.nearBlackMaximum()) near++;
        if (luminance >= policy.brightMinimum()) bright++;
        sum += luminance;
    ...
    if (b < policy.minimumBlackRatio()) warnings.add("INSUFFICIENT_BLACK_BACKGROUND");
    if (n < policy.minimumNearBlackRatio()) warnings.add("GRAY_HAZE_OR_BRIGHT_BACKGROUND");
    if (h > policy.maximumBrightRatio()) warnings.add("EXCESSIVE_BRIGHT_AREA");
    if (h < policy.minimumHighlightCoverage()) warnings.add("NO_MEASURABLE_FOCAL_HIGHLIGHT");
    String classification =
        warnings.isEmpty()
            ? "AMOLED_SUITABLE"
    ...
    for (int i = 0; i < histogram.length; i++) {
      if (seen < (count + 1) / 2 && seen + histogram[i] >= (count + 1) / 2) median = i / 255.0;
      seen += histogram[i];
