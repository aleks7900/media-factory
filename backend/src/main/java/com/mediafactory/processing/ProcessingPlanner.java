...
public class ProcessingPlanner {
  ...
  public int scale(int w, int h, Map<String, Object> p) {
    ...
    if (factor == 0) throw new IllegalArgumentException("Target requires more than 4x upscale");
  ...
  public Map<String, Object> plan(
    ...
    if (!profiles.stream().map(p -> p.get("key").toString()).toList().containsAll(manual.keySet()))
    ...
    for (var p : profiles) {
      ...
      if (manual.containsKey(p.get("key"))) node.put("manualCrop", manual.get(p.get("key")));
      nodes.add(node);
    }
    return Map.of(
        "sourceAssetId",
  ...
  private int branchScale(int width,int height,Map<String,Object> profile,Map<String,Object> manual) {
    var definition=map(profile.get("definition"));
    if(!manual.containsKey(profile.get("key")))return scale(width,height,definition);
    var rectangle=map(manual.get(profile.get("key")));
    return scale(Math.max(1,(int)Math.floor(width*number(rectangle,"width",1))),Math.max(1,(int)Math.floor(height*number(rectangle,"height",1))),definition);
  }
}
