...
public class OpenAiImageClient implements AutoCloseable {
 ...
 private final OpenAiImageProperties secrets;private final ImageGenerationProperties properties;private final HttpClient client;
 public OpenAiImageClient(OpenAiImageProperties secrets,ImageGenerationProperties properties) {
  this.secrets=secrets;this.properties=properties;
  client=HttpClient.newBuilder().connectTimeout(properties.provider("openai").timeout().connect()).followRedirects(HttpClient.Redirect.NEVER).build();
 }
 public Response generate(String json,String operationId) {
  if(secrets.apiKey().isBlank()) throw new ImageGenerationException(Type.AUTHENTICATION,"Image provider credentials are not configured");
  Duration timeout=properties.provider("openai").timeout().request();
  var request=HttpRequest.newBuilder(secrets.endpoint()).timeout(timeout).header("Authorization","Bearer "+secrets.apiKey())
  ...
  try {
   ...
   if(id!=null && (id.length()>200||!id.matches("[a-zA-Z0-9_\\-\\[\\]]+"))) id=null;
   if(response.statusCode()!=200) throw OpenAiImageExceptionMapper.map(response.statusCode(),response.body(),response.headers().firstValue("retry-after").orElse(null),id);
   var headers=new HashMap<String,String>();
   response.headers().firstValue("x-ratelimit-remaining-requests").filter(v->v.matches("[0-9]{1,10}")).ifPresent(v->headers.put("remainingRequests",v));
   response.headers().firstValue("x-ratelimit-reset-requests").filter(v->v.matches("[0-9.hms]{1,40}")).ifPresent(v->headers.put("requestReset",v));
   ...
   if(cause instanceof IllegalStateException) throw new ImageGenerationException(Type.UNEXPECTED,"Provider response exceeds the permitted size",Duration.ZERO,true,null);
   throw new ImageGenerationException(Type.UNAVAILABLE,"Image provider connection failed",Duration.ZERO,true,null);
  }
 }
 ...
 public static final class BoundedBody implements HttpResponse.BodySubscriber<byte[]> {
  ...
  public void onNext(List<ByteBuffer> buffers) {
   for(var buffer:buffers) {
    if(bytes.size()+(long)buffer.remaining()>limit) { subscription.cancel();body.completeExceptionally(new IllegalStateException("Response too large"));return; }
