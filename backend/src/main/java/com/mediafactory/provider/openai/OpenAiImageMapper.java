...
final class OpenAiImageMapper {
  ...
  static String request(Request request) {
    ...
    body.put("quality", o.quality().name().toLowerCase(Locale.ROOT));
    body.put("output_format", o.format().name().toLowerCase(Locale.ROOT));
    body.put("background", o.transparentBackground() ? "transparent" : "opaque");
    return JSON.writeValueAsString(body);
  }

  static Result<Media> response(OpenAiImageClient.Response response, Request request,
      OpenAiImageProperties secrets) {
    try {
      var root = JSON.readTree(response.body());
      var images = root.path("data");
      ...
      if (!revised.isBlank()) {
        metadata.put("revisedPrompt",
            secrets.redact(revised.substring(0, Math.min(revised.length(), 32000))));
      }
      if (root.path("created").isIntegralNumber()) {
        metadata.put("providerCreated", root.path("created").asText());
      }
      var usage = root.path("usage");
      var details = usage.path("input_tokens_details");
      addNumber(metadata, "textInputTokens", details.path("text_tokens"));
      addNumber(metadata, "imageInputTokens", details.path("image_tokens"));
      addNumber(metadata, "outputTokens", usage.path("output_tokens"));
      addNumber(metadata, "inputTokens", usage.path("input_tokens"));
      metadata.put("format", request.options().format().name());
      metadata.put("quality", request.options().quality().name());
      return new Result<>(new Media(bytes,
          request.options().format() == ImageOptions.Format.PNG ? "image/png" : "image/jpeg"),
          new Usage("openai", request.options().model(), "IMAGE_GENERATION",
              usage.path("input_tokens").asLong(0), usage.path("output_tokens").asLong(0), null,
              "USD"), Map.copyOf(metadata));
    } catch (Exception e) {
      throw new ImageGenerationException(Type.UNEXPECTED,
          "Image provider returned an invalid image response", Duration.ZERO, true,
          response.requestId());
    }
  }

  private static void addNumber(Map<String, String> target, String key,
