...
(
    ...
    block_duplicates     boolean     NOT NULL,
    block_near           boolean     NOT NULL,
...
VALUES ('WALLPAPER', 1, 4, 10, .94, .86, true, false, 'WARN', .80),
...
ALTER TABLE collections
    ADD COLUMN similarity_profile varchar(40) NOT NULL DEFAULT 'WALLPAPER' REFERENCES similarity_profiles (id);
...
(
    ...
    preprocessing varchar(500) NOT NULL,
...
RETURN NEW;
END $$;
CREATE TRIGGER immutable_model_identity
    BEFORE UPDATE
    ON embedding_models
    FOR EACH ROW EXECUTE FUNCTION protect_model_identity();
INSERT INTO embedding_models
VALUES ('00000000-0000-0000-0000-000000000501', 'local-clip', 'openai/clip-vit-base-patch32',
        '3d74acf9a28c67741b2f4f2ea7635f0aaf6f0268', 512,
        'RGB/bicubic-shortest224/center224/rescale255/CLIP-mean-std', true, now());
...
(
    ...
    created_at        timestamptz  NOT NULL DEFAULT now(),
    UNIQUE (asset_id, type, algorithm_version)
);
...
SELECT gen_random_uuid(), id, 'SHA256', sha256, 'SHA-256', '1'
FROM assets;
...
(
    ...
    CHECK (abs(vector_norm(embedding) - 1) < 0.001)
);
...
(
    ...
    model_id                 uuid          NOT NULL REFERENCES embedding_models (id),
    profile_id               varchar(40)   NOT NULL REFERENCES similarity_profiles (id),
    profile_snapshot         jsonb         NOT NULL,
    sha256_match             boolean       NOT NULL,
    phash_distance           integer CHECK (phash_distance BETWEEN 0 AND 64),
    embedding_similarity     float8 CHECK (embedding_similarity BETWEEN -1.00001 AND 1.00001),
    context                  jsonb         NOT NULL,
    automatic_classification varchar(40)   NOT NULL,
    human_classification     varchar(40),
    ...
    UNIQUE (source_asset_id, target_asset_id, model_id, profile_id)
);
...
CREATE TABLE similarity_evaluation_history
(
    id            uuid PRIMARY KEY,
    comparison_id uuid        NOT NULL REFERENCES similarity_comparisons (id),
...
(
    ...
    created_at    timestamptz NOT NULL DEFAULT now(),
    UNIQUE (comparison_id, asset_id)
);
CREATE TABLE similarity_review_actions
(
    id            uuid PRIMARY KEY,
    comparison_id uuid REFERENCES similarity_comparisons (id),
    group_id      uuid,
...
(
    id                 uuid PRIMARY KEY,
    model_id           uuid        NOT NULL REFERENCES embedding_models (id),
    type               varchar(30) NOT NULL CHECK (type IN ('EXACT', 'PERCEPTUAL', 'NEAR_DUPLICATE')),
    status             varchar(20) NOT NULL DEFAULT 'OPEN',
    canonical_asset_id uuid REFERENCES assets (id),
    revision           integer     NOT NULL DEFAULT 0,
    created_at         timestamptz NOT NULL DEFAULT now(),
    updated_at         timestamptz NOT NULL DEFAULT now()
);
ALTER TABLE similarity_review_actions
    ADD FOREIGN KEY (group_id) REFERENCES duplicate_groups (id);
...
(
    ...
    model_id          uuid         NOT NULL REFERENCES embedding_models (id),
    parent_id         uuid REFERENCES similarity_jobs (id),
...
RETURN NEW;
END $$;
CREATE TRIGGER asset_similarity_enqueue
    AFTER INSERT
    ON assets
    FOR EACH ROW EXECUTE FUNCTION enqueue_asset_similarity();
ALTER TABLE provider_permits
    ADD COLUMN similarity_job_id uuid UNIQUE REFERENCES similarity_jobs (id);
ALTER TABLE provider_permits DROP CONSTRAINT permit_owner;
ALTER TABLE provider_permits
    ADD CONSTRAINT permit_owner CHECK (num_nonnulls(job_id, qa_job_id, similarity_job_id) = 1);
CREATE TABLE embedding_compute_usage
(
    id                     uuid PRIMARY KEY,
    job_id                 uuid         NOT NULL REFERENCES similarity_jobs (id),
    attempt                integer      NOT NULL,
    provider               varchar(80)  NOT NULL,
    model                  varchar(200) NOT NULL,
    version                varchar(100) NOT NULL,
    operation              varchar(40)  NOT NULL DEFAULT 'IMAGE_EMBEDDING',
    input_usage            integer      NOT NULL,
    output_usage           integer,
    duration_ms            bigint,
    device                 varchar(30),
    external_api_cost      numeric      NOT NULL DEFAULT 0,
...
(
    id                      uuid PRIMARY KEY,
    run_id                  uuid    NOT NULL REFERENCES collection_clustering_runs (id),
    ordinal                 integer NOT NULL,
    centroid                vector,
    representative_asset_id uuid REFERENCES assets (id),
    outlier                 boolean NOT NULL,
    UNIQUE (run_id, ordinal)
);
CREATE TABLE collection_cluster_members
(
    cluster_id           uuid NOT NULL REFERENCES collection_clusters (id),
    asset_id             uuid NOT NULL REFERENCES assets (id),
...
(
    batch_id      uuid    NOT NULL REFERENCES generation_batches (id),
    generation_id uuid    NOT NULL UNIQUE REFERENCES generations (id),
    ordinal       integer NOT NULL,
    PRIMARY KEY (batch_id, ordinal)
);
CREATE TABLE generation_batch_actions
(
    id             uuid PRIMARY KEY,
    batch_id       uuid          NOT NULL REFERENCES generation_batches (id),
    actor          varchar(200)  NOT NULL,
...
ALTER TABLE quality_findings
    ADD CONSTRAINT quality_findings_source_check CHECK (source IN
                                                        ('TECHNICAL', 'VISION_MODEL', 'HUMAN',
                                                         'SIMILARITY'));

-- Defense in depth: strict collections require complete active-model analysis and no blocking pair.
CREATE FUNCTION similarity_publication_block_reason(asset uuid) RETURNS text
    LANGUAGE plpgsql AS $$
DECLARE
profile similarity_profiles; active_model
uuid;
BEGIN
SELECT p.*
INTO profile
FROM assets a
         ...
         JOIN similarity_profiles p ON p.id = col.similarity_profile
WHERE a.id = asset;
SELECT id
INTO active_model
FROM embedding_models
WHERE active;
IF
profile.id='STOCK_STRICT' AND NOT EXISTS(SELECT 1 FROM similarity_jobs WHERE asset_id=asset AND model_id=active_model AND type='GENERATE_ASSET_EMBEDDING' AND status='SUCCEEDED') THEN
 RETURN 'Similarity analysis incomplete for strict publication';
END IF;
 IF
EXISTS(SELECT 1 FROM similarity_comparisons s WHERE model_id=active_model AND (source_asset_id=asset OR target_asset_id=asset)
 ...
 RETURN 'Similarity policy blocks publication';
END IF;
RETURN null;
END $$;
CREATE FUNCTION require_similarity_publication() RETURNS trigger
    LANGUAGE plpgsql AS $$
DECLARE
asset uuid; reason
text;
BEGIN
 IF
TG_TABLE_NAME='generations' THEN
   IF NEW.status<>'PUBLISHED' THEN RETURN NEW;
END IF;
SELECT id
INTO asset
FROM assets
WHERE generation_id = NEW.id;
ELSE asset:=NEW.asset_id;
END IF;
 reason
:=similarity_publication_block_reason(asset);
 IF
reason IS NOT NULL THEN RAISE EXCEPTION '%',reason;
END IF;
RETURN NEW;
END $$;
...
CREATE TRIGGER generation_similarity
    ...
    ON generations
    FOR EACH ROW EXECUTE FUNCTION require_similarity_publication();
