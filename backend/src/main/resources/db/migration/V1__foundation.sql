...
(
    ...
    status     varchar(24) NOT NULL CHECK (status IN
                                           ('CREATED', 'QUEUED', 'GENERATING', 'GENERATED',
                                            'QA_PENDING', 'APPROVED', 'REJECTED', 'FAILED',
                                            'PUBLISHED')),
    prompt     text        NOT NULL,
...
(
    id         uuid PRIMARY KEY,
    asset_id   uuid        NOT NULL REFERENCES assets (id),
    kind       varchar(20) NOT NULL CHECK (kind IN ('TECHNICAL', 'HUMAN')),
    decision   varchar(20) NOT NULL CHECK (decision IN ('PASSED', 'APPROVED', 'REJECTED')),
...
(
    ...
    input_usage    bigint         NOT NULL CHECK (input_usage >= 0),
    output_usage   bigint         NOT NULL CHECK (output_usage >= 0),
    estimated_cost numeric(18, 8) NOT NULL CHECK (estimated_cost >= 0),
    currency       char(3)        NOT NULL,
    created_at     timestamptz    NOT NULL DEFAULT now(),
    UNIQUE (job_id, attempt, operation)
);
