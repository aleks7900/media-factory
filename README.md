...
Open **Embedding Jobs → Backfill originals** for existing assets, then **Collection Diversity → Analyze collection**. Newly generated originals enqueue similarity work automatically. **Duplicate Review** provides side-by-side evidence, human classification, and canonical selection; **Similarity Explorer** supports image and text retrieval.
